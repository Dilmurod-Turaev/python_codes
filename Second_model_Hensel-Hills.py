...
for j in range(M):
    ex_sol[j] = p[j, 0]
#print("p = ",p)
ex_sol = [float(i) for i in ex_sol]
...
for i in range(d, 0, -1):
    F2_0 = (1 / (2 * tau) - lam_v / (tau ** 2)) * p2[0, i] + ((2 * lam_v) / (tau ** 2) + (2 * kap * lam_p) / (tau * h ** 2)) * p2[1, i] - (kap * lam_p) / (tau * h ** 2) * (p2[1, i+1] + p2[1, i-1])
    p2[1, i-1] = (C2 / A2) * p2[1, i] - p2[1, i+1] - F2_0 / A2
    
    for j in range(2, M-1):
        p2[j, i-1] = (p2[j+1,i]*h**2*lam_v + p2[j+1,i]*h**2*tau/2 + p2[j-1,i-1]*kap*lam_p*tau + p2[j-1,i]*h**2*lam_v - p2[j-1,i]*h**2*tau/2 - 2*p2[j-1,i]*kap*lam_p*tau + p2[j-1,i+1]*kap*lam_p*tau - 2*p2[j,i]*h**2*lam_v + 2*p2[j,i]*kap*lam_p*tau + 2*p2[j,i]*kap*tau**2 - p2[j,i+1]*kap*lam_p*tau - p2[j,i+1]*kap*tau**2)/(kap*tau*(lam_p + tau))

    F2_M = (1 / (2 * tau) - lam_v / (tau ** 2)) * p2[M-3, i] + ((2 * lam_v) / (tau ** 2) + (2 * kap * lam_p) / (tau * h ** 2)) * p2[M-2, i] - (kap * lam_p) / (tau * h ** 2) * (p2[M-2, i+1] + p2[M-2, i-1])
    p2[M-1, i-1] = (C2 / A2) * p2[M-1, i] - p2[M-1, i+1] - F2_M / A2
#print("p2 = ", p2)
for j in range(M):
    ap_sol[j] = p2[j, 0]
ap_sol = [float(i) for i in ap_sol]
