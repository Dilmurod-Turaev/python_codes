...
for i in range(d, 0, -1):
    for j in range(2, M-1):
        F2_0 = p2[0, i] - kap * lam_p * (p2[0, i + 1] - 2 * p2[0, i] + p2[0, i - 1]) / h ** 2
        p2[1, i-1] = (C2 / A2) * p2[1, i] - p2[1, i+1] - F2_0 / A2

        p2[j, i-1] = (h**2) * (p2[j+1, i] - p2[j-1, i]) / (2 * kap * (tau + lam_p)) + 2 * p2[j, i] - p2[j, i+1] + lam_p * (p2[j-1, i-1] - 2 * p2[j-1, i] + p2[j-1, i+1]) / (tau + lam_p)

        F2_M = p2[M-2, i] - kap * lam_p * (p2[M-2, i + 1] - 2 * p2[M-2, i] + p2[M-2, i - 1]) / h ** 2
